 ...
 * ハイブリッド投稿生成ワークフロー
 *
 * Gemini（生成・高速・安価）+ Claude（評価・高精度）
 *
 * フロー:
 * RESEARCH → DRAFT(Gemini) → REVIEW(Claude) → REVISE(Gemini) → POLISH(Gemini)
 */
...
/**
 * REVIEW: Claude で品質スコアを評価（高精度）
 *
 * ★ ハイブリッドのキモ - Claudeの高い推論能力で厳格評価
 */
async function reviewNodeClaude(
  state: PostGeneratorStateType
): Promise<Partial<PostGeneratorStateType>> {
...
- feedbackは具体的に（「〇〇を△△に変えると良い」）`;
  ...
  try {
    // ★ Claude で評価（高精度）
    const content = await sendClaudeMessage(prompt, {
      model: 'sonnet',
      temperature: 0.3, // 評価は低めの温度で一貫性を
      maxTokens: 1024,
    });
    ...
    if (!jsonMatch) {
      return createDefaultReviewResult('評価の解析に失敗しました。');
    }
    ...
    return createDefaultReviewResult('Claude APIエラー。Geminiにフォールバック。');
  }
}

function createDefaultReviewResult(message: string): Partial<PostGeneratorStateType> {
  return {
    score: {
      empathy: 2, benefit: 1, cta: 1, credibility: 1, urgency: 0,
      originality: 0, engagement: 0, scrollStop: 0, total: 5,
      strengths: [], weaknesses: [message]
    },
    feedback: message,
...
export function createHybridPostGeneratorGraph() {
  const workflow = new StateGraph(PostGeneratorState)
    .addNode('research', researchNode)
    .addNode('draft', draftNode)
    .addNode('review', reviewNodeClaude)  // ★ Claude で評価
...
/**
 * 単一投稿を生成（ハイブリッド: Gemini + Claude）
 */
export async function generateSinglePostHybrid(
  account: string,
  ...
  const graph = createHybridPostGeneratorGraph();
  ...
  const defaultScore: QualityScore = {
    empathy: 0, benefit: 0, cta: 0, credibility: 0, urgency: 0,
    originality: 0, engagement: 0, scrollStop: 0, total: 0,
 ...
 * 複数投稿を一括生成（ハイブリッド版）
 */
export async function generateMultiplePostsHybrid(
  count: number,
  account: string,
  accountType: 'ライバー' | 'チャトレ',
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      const result = await generateSinglePostHybrid(
        account,
        accountType,
        target,
        benefit,
